...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4, qkv_bias=True):
        ...
        self.drop_path = DropPath(0.1, inplace=True)
        ...
        for lvl in [min(i + 2, self.max_lvl) for i in range(4)]:
            if lvl == patch_lvl or lvl < self.min_lvl:
                ...
                while stride > 1:
                    ...
                    layers += [nn.BatchNorm2d(embed_dim), nn.GELU()] if stride > 2 else []
                    stride /= 2
...
def vit_encoder(depth, embed_dim, num_heads, out_dim=256, image_size=1024):
...
tap_vit_b_encoder = partial(vit_encoder, depth=12, embed_dim=768, num_heads=12)
tap_vit_l_encoder = partial(vit_encoder, depth=24, embed_dim=1024, num_heads=16)
