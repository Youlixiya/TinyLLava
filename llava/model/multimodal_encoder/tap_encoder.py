...
class TAPImageBaseProcessor(BaseProcessor):
...
class TAPImageProcessor(BaseProcessor):
...
class TAPVisionTower(nn.Module):
    ...
    def load_model(self):
        ...
        self.tap = model_registry[self.args.tap_model_type](checkpoint=self.args.tap_checkpoint)
        self.tap.concept_projector.reset_weights(self.args.concept_weights)
        self.tap.text_decoder.reset_cache(max_batch_size=8)
        self.vision_tower = self.tap.image_encoder
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            image_feature = self.vision_tower(images.to(device=self.device, dtype=self.dtype))[0]
