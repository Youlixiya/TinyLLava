...
# class TinyLisaMetaModel:
#     def __init__(
#         self,
#         config,
#         **kwargs,
#     ):
#         super(TinyLisaMetaModel, self).__init__(config)
...
class TinyLISAForCausalLM(LlavaForConditionalGeneration):
    def __init__(
        self,
        config,
        **kwargs,
    ):
        super(TinyLISAForCausalLM, self).__init__(config, **kwargs)
        self.config = config
        if not hasattr(self.config, "train_mask_decoder"):
            self.config.train_mask_decoder = kwargs["train_mask_decoder"]
            self.config.out_dim = kwargs["out_dim"]
